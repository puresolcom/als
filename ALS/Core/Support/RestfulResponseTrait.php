...
{
    protected function response($content, $status = 200, $contentType = 'application/json', $headers = [])
    {
        $headers = array_merge(['Content-Type' => $contentType], $headers);
        return new Response($content, $status, $headers);
    }

    protected function jsonResponse($content, $message = 'OK', $status = 200, $errors = [])
    {
        $formattedContent = [
            'api'    => [
                'format' => 'json'
